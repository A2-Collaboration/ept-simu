...
c      Programm erwuerfelt Brems-Ereignisse fuer feste
c      Photonenenergien. Fragt nach den
...
c      das in brems_mc.dat ausgegeben wird. Man erhaelt also
c      ein Histogramm Elektronenort auf der "Bildlinie" fuer
c      feste Photonenenergien.
c
...
c
      ...
      xr0=11.5   ! Lage des Radiators vor der Polkante in cm
      zz=28.     ! Ladungszahl Radiator
      ab=-.184   ! Bildgerade, alternativ (0,0) in Index
      bb=17.9458
      ...
      read*,zz
      print*,'Bildlinie a,b in Index ',ab,bb,' geometrie [0,2.82]:'
      ab=0.
      bb=2.82
      read*,ab,bb
c      Gesamt-Laenge der Bildlinie zwischen index_x 0 und 90 fuer Hist
 10   d0=sqrt(90**2+(90.*ab)**2)
      ...
      x0=x0-xr0/incr/tan(alpha0) ! Radiator Sollort fuer xr0/cm
      z0=z0-xr0/incr             ! vor Polkante
      print*,'Startpunkt bei ',x0,z0
...
c          xyr ist hier die Abweichung vom Sollwert in cm
...
c          print*,'Winkel-El./rad [*]:'
 ...
 11       continue
     ...
     &        hist(i)/izug/histep*.002
        ...
        write(17,*)e,ssum,sig/incr
        print*,'Ergebnis Apfu in brems_mc.dat fuer e0 und e/MeV',
     &    e0,e
        print*,'gemittelte Ergebnisse in cm und Index_x'
        print*,'Schwerpunkt ',ssum*incr, ssum
        print*,'sigma       ',sig,sig/incr
        ...
        print*,'und in fort.17 in Index-Einheiten als '
        print*,'E/MeV,Schwerpunkt,sigma'
      end do
      close(1)
      stop
      end
